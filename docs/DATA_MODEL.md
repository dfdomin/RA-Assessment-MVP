...
**Fecha**: 2026-05-15  
**Referencia PRD**: §7 (Modelo de Datos), §F10–F15 (impactos en v2.1)  
...
| `email` | `VARCHAR(254)` | No | — | Email institucional (unique) |
...
Períodos académicos (ej. "TGA RA1 2024-2"). Un período evalúa un SO/RA con una rúbrica específica.
...
**Índices**: `UNIQUE(name)`, `INDEX(student_outcome_id)`, `INDEX(status)`
...
Módulo = asignación de un curso-grupo a un período. Hasta 15 módulos por período.
...
| `submitted_at` | `TIMESTAMPTZ` | Sí | `NULL` | Timestamp del submit final del docente |

**Índices**: `INDEX(period_id)`, `INDEX(status)`
...
Análisis cualitativo del **docente** por PI de su módulo (Nivel 1 ABET). Obligatorio para estado "Completado".
...
**Índices**: `UNIQUE(module_id, perf_indicator_id)`

---

### 3.15 `leader_analysis`

Síntesis del **líder** por período y PI (Nivel 2 ABET). Obligatorio para exportar el reporte final.
...
**Índices**: `UNIQUE(period_id, perf_indicator_id)`

---

### 3.16 `action_plans`

Plan de acción del líder por período y PI ("Closing the Loop" ABET — Sección 4 del reporte).
...
### 3.21 `leader_report_drafts` (nueva en v2.1)

Borradores de conclusiones del líder por período para el informe de F14.

| Columna | Tipo | Nulo | Descripción |
|---|---|---|---|
| `id` | `SERIAL` | No | PK |
| `period_id` | `INT` | No | FK → `periods.id` (UNIQUE) |
| `pi_conclusions` | `JSONB` | No | `{"pi_id_1": "texto conclusión", …}` |
| `last_updated` | `TIMESTAMPTZ` | No | `DEFAULT NOW()` |
| `updated_by` | `INT` | No | FK → `users.id` |

**Índices**: `UNIQUE(period_id)`
